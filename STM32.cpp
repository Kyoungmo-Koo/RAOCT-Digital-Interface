...
//Scan and Dimension
#define RASTER_DIM1             4
#define RASTER_DIMx25           5

int open_file(int scan_pattern) {
    if (scan_pattern == RASTER_DIM1) {
        int test = 0;
        return 1234; //placeholder values
    }
    else if (scan_pattern == RASTER_DIMx25) {
        int test = 1;
        return 2345;
    }
    return 0;
...
int test(int argc, char** argv)
{
    ...
    int num_position = open_file(RASTER_DIM1);
    // Open the csv file to record position feedback
    fopen_s(&fp, "data.csv", "w+");
    // Read txt data
    fopen_s(&fr, "S:\\Digital_Interface\\EnginePattern_raster_amp1_res512_hexadecimal_downsample.txt", "r");
