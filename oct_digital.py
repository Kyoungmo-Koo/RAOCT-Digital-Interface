...
from vortex.engine import Source, EngineConfig, Engine, StackDeviceTensorEndpointInt8 as StackDeviceTensorEndpoint, AscanStreamEndpoint, CounterStreamEndpoint, GalvoTargetStreamEndpoint, GalvoActualStreamEndpoint, MarkerLogStorage, NullEndpoint, RadialDeviceTensorEndpointInt8 as RadialDeviceTensorEndpoint
from vortex.format import FormatPlanner, FormatPlannerConfig, StackFormatExecutorConfig, StackFormatExecutor, SimpleSlice, RadialFormatExecutor, RadialFormatExecutorConfig
from vortex.marker import Flags, VolumeBoundary, ScanBoundary
from vortex.scan import RasterScanConfig, RadialScanConfig, FreeformScanConfig, FreeformScan, SpiralScanConfig, SpiralScan, limits, inactive_policy
...
from vortex_tools.ui.display import RasterEnFaceWidget, RadialEnFaceWidget, CrossSectionImageWidget
...
class Config:
    ...
    trigger_delay_seconds=400e-9


    blocks_to_allocate=128
    preload_count=32

    swept_source=Source(200_000, 2858, 0.5, 1) # source.ThorlabsVCSEL400k
...
class System(QWidget):
    ...
    def _setup_scan(self):
        ...
        # rsc.offset = [-0.075, -0.2] # 0.25
        rsc.offset = [-0.025, -0.1] # 1
        rsc.angle = np.pi
        ...
        rsc.set_half_evenly_spaced(self._cfg.bscans_per_volume)
        rsc.bidirectional_segments = self._cfg.bidirectional
        rsc.bidirectional_volumes = self._cfg.bidirectional
        rsc.limits = [limits.ScannerMax_Saturn_5B]*2
        rsc.samples_per_second = self._cfg.swept_source.triggers_per_second
        rsc.flags = RADIAL_FLAGS
        self._radial_config = rsc

        asc = RadialScanConfig()
        asc.bscans_per_volume = self._cfg.bscans_per_volume
        asc.ascans_per_bscan = self._cfg.ascans_per_bscan
        asc.volume_extent = Range(0, math.pi)
    ...
    def _update_scan(self):
        if RASTER_SCAN:
            primary_segments = self._raster_config.to_segments()
        else:
            primary_segments = self._radial_config.to_segments()
        aiming_segments = self._aiming_config.to_segments()

        pattern = []
        if self._use_aiming and self._use_raster:
            idx = np.linspace(0, len(primary_segments), self._cfg.aiming_count, dtype=int)
            for (i, (a, b)) in enumerate(zip(idx[:-1], idx[1:])):
                if i > 0:
                    markers = primary_segments[a].markers
                    markers.insert(0, VolumeBoundary(0, 0, False))
                    markers.insert(0, ScanBoundary(0, 0))

                pattern += primary_segments[a:b]
                pattern += aiming_segments
        elif self._use_aiming:
            pattern = aiming_segments
        else:
            pattern = primary_segments
    ...
    def _setup_core(self):
        ...
        ioc_in.channels.append(AnalogVoltageInput('Dev1/ai0', 25 / 10, stream, 0))
        ioc_in.channels.append(AnalogVoltageInput('Dev1/ai1', 25 / 10, stream, 1))
    ...
    def _setup_engine(self):
        ...
        ec.add_processor(self._process, [self._generic_format, self._aiming_format, self._raster_format, self._radial_format])
        ...
        ec.add_formatter(self._radial_format, [self._radial_tensor_endpoint])
    ...
    def _setup_display(self):
        ...
        fc.mask = self._radial_config.flags
        fc.segments_per_volume = self._radial_config.bscans_per_volume
        fc.records_per_segment = self._radial_config.ascans_per_bscan
        radial_format = FormatPlanner(gcl('format-radial', self._cfg.log_level))
        ...
        cfec.sample_slice = SimpleSlice(50, 250)
        ...
        rfec.volume_xy_extent = [Range(self._cfg.scan_dimension, -self._cfg.scan_dimension), Range(-self._cfg.scan_dimension, self._cfg.scan_dimension)]
        rfec.segment_rt_extent = (self._radial_config.bscan_extent, self._radial_config.volume_extent)
        rfec.radial_segments_per_volume = self._radial_config.bscans_per_volume
        rfec.radial_records_per_segment = self._radial_config.ascans_per_bscan
        ...
        self._radial_tensor_endpoint = RadialDeviceTensorEndpoint(rfe, (self._radial_config.bscans_per_volume, self._radial_config.ascans_per_bscan, samples_to_save), get_logger('endpoint-radial', self._cfg.log_level))
    ...
    def _setup_interface(self):
        ...
        self._raster_widget = RasterEnFaceWidget(self._raster_tensor_endpoint, range=[45, 60], debug=False)
        self._raster_widget.setStyleSheet('background-color: black;')
        self._raster_widget.resize(500, 500)

        self._radial_widget = RadialEnFaceWidget(self._radial_tensor_endpoint, range=[45, 60], debug=False)
        self._radial_widget.setStyleSheet('background-color: black;')
        self._radial_widget.resize(500, 500)
        ...
        self._flythrough_widget = CrossSectionImageWidget(self._raster_tensor_endpoint, sizing=[NumpyImageWidget.Sizing.Stretch]*2, range=[30, 50], debug=False)
        ...
        def _radial_handler(idxs):
            self._radial_widget.notify_segments(idxs)
        self._radial_tensor_endpoint.aggregate_segment_callback = _radial_handler
        ...
        if RASTER_SCAN:
            ...
            panel.addWidget(self._radial_widget, 2)
        ...
        for (label, shift_) in [('<', (0, 1)), ('v', (-1, 0)), ('o', None), ('^', (1, 0)), ('>', (0, -1))]:
            ...
            def _click_handler(shift):
                if shift is None:
                    ...
                    self._raster_config.offset = self._raster_config.offset + 0.025 * np.asanyarray(shift)
                self._aiming_config.offset = self._raster_config.offset
                print(self._raster_config.offset)
                self._update_scan()
                self._engine.scan_queue.interrupt(self._scan)
        ...
        rcbx = QCheckBox('Raster' if RASTER_SCAN else 'Radial')
    ...
    def start_engine(self):
        ...
        # restart the scan
        waveform = self._scan.scan_buffer()
        self._engine.scan_queue.reset(0, waveform[0], (waveform[1] - waveform[0]) * self._scan.config.samples_per_second)
        self._engine.scan_queue.interrupt(self._scan)
