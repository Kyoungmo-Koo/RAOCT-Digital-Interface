...
from vortex.engine import EngineConfig, Engine, StackDeviceTensorEndpointInt8 as StackDeviceTensorEndpoint, AscanStreamEndpoint, CounterStreamEndpoint, GalvoTargetStreamEndpoint, GalvoActualStreamEndpoint, MarkerLogStorage, NullEndpoint, Source
...
from vortex_tools.ui.display import RasterEnFaceWidget, CrossSectionImageWidget, Sizing


# from igmr_oct_toolkit.widget import VolumeImageWidget
# from igmr_oct_toolkit.viz.math import scale, translate
...
class System(QWidget):
    ...
    def _setup_core(self):
        ...
        if self._cfg.internal_clock:
            from pathlib import Path
            clock_path = Path(r'S:\Digital_Interface_20241212\Digital_Interface\STM32\clock.npz')
    ...
    def _setup_display(self):
        ...
        # cfec.sample_slice = SimpleSlice(50 + 30, 50 + 120 )
    ...
    def _setup_interface(self):
        ...
        self._raster_widget = RasterEnFaceWidget(self._raster_tensor_endpoint, debug=False, range=[24, 64])
        self._raster_widget.setStyleSheet('background-color: black;')
        self._raster_widget.resize(500, 500)
        # self._flythrough_widget = CrossSectionImageWidget(self._raster_tensor_endpoint, sizing=[Sizing.Stretch]*2, cmap=cm.gray, range=[0, 30], debug=False)
        self._flythrough_widget = CrossSectionImageWidget(self._raster_tensor_endpoint, sizing=[Sizing.Stretch]*2, debug=False)
        ...
        def _enface_click(e: QMouseEvent):
            ...
            xform.translate(-w.pixmap.width() / 2.0, -w.pixmap.height() / 2.0)
        ...
        self._aiming_widgets = [CrossSectionImageWidget(self._aiming_tensor_endpoint, fixed=i, sizing=[Sizing.Stretch]*2, debug=False) for i in range(self._aiming_config.bscans_per_volume)]
        ...
        self._dispersion_spin.setMinimum(-1000)
        self._dispersion_spin.setMaximum(1000)
    ...
    def start_engine(self):
        ...
        # restart the scan
        self._engine.scan_queue.reset(0, self._scan.scan_segments()[0].entry_position, self._scan.scan_segments()[0].entry_velocity(self._scan.config.samples_per_second))
        self._engine.scan_queue.interrupt(self._scan)
