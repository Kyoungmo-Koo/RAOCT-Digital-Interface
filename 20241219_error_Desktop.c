...
int test(int argc, char** argv)
{
    ...
    while (1) {
        if (_kbhit()) {
            ...
            if (state == SET_STATE && ch == (KB_CONSTANT + 7)) {
                state = STREAM_STATE;
                msg = ((uint32_t)state << 16) | (uint16_t)payload;
                status = WriteFile(hComm, &msg, 4, &bytesWrite, NULL);
                i = 2;
            ...
            if ((state == STREAM_STATE || state == 7) && ch == (KB_CONSTANT + STOP_STATE)) {
                state = STOP_STATE;
            }

            if ((state == STREAM_STATE || state == 7) && ch == (KB_CONSTANT + PING_STATE)) {
                ping = 1;
            }
        ...
        if (state == STREAM_STATE || state == 7 || state == STOP_STATE) {
            ...
            if (i % 2 == 1) {
                ...
                if ((state == STREAM_STATE || state == 7) && idle_sent == 0) {
            ...
            if (state == STREAM_STATE || state == 7 || state == STOP_STATE) {
                ...
                else if (state == 7 || state == STOP_STATE) {
                    for (int j = 0; j < MULTIPLIER * NUM_OF_POSITIONS_PER_CHUNK; j += MULTIPLIER) {
                        x = rx_buffer[j + 3] * 256 + rx_buffer[j + 2];
                        y = rx_buffer[j + 1] * 256 + rx_buffer[j];
                        fprintf(fp, "%d,%d,", x, y);
